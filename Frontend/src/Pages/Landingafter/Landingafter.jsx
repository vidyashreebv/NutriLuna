...
function Landingafter() {
    ...
    return (
        <div className="landingafter-container">
            <Navbarafter navItems={navItems} />

            <div className="landingafter-carousel">
                {data.map((slide, index) => (
                    <div
                        key={index}
                        className={`landingafter-carousel-slide ${index === currentSlide ? 'active' : ''}`}
                        style={{ backgroundImage: `url(${slide.image})` }}
                    >
                        <div className="landingafter-carousel-content">
                            <div className="landingafter-content-place">{slide.place}</div>
                            <div className="landingafter-content-title-1">{slide.title}</div>
                            <div className="landingafter-content-title-2">{slide.title2}</div>
                            <div
                                className="landingafter-content-description"
            ...
            <button className="landingafter-carousel-prev" onClick={prevSlide}>&#10094;</button>
            <button className="landingafter-carousel-next" onClick={nextSlide}>&#10095;</button>

            <div className="landingafter-carousel-dots">
                {data.map((_, index) => (
                    <span
                        key={index}
                        className={`landingafter-dot ${index === currentSlide ? 'active' : ''}`}
